...
- `/static/*` is served through Django/WhiteNoise from the build-time `staticfiles/` output generated by `collectstatic`.
- Vercel routes static requests to the WSGI app so WhiteNoise can serve collected files inside the serverless bundle.
- `build.sh` runs `collectstatic --clear` so Django admin and app assets are prepared during build.
